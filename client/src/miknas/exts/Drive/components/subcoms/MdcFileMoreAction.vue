<template>
  <q-btn v-if="fileInfo.isFile" dense flat round icon="more_vert" @click.stop.prevent="">
    <q-menu auto-close>
      <q-list>
  ...
  <q-btn v-else dense flat round icon="more_vert" @click.stop.prevent="">
    <q-menu auto-close>
      <q-list>
        <q-item clickable @click="fileOp.removeFile(fileInfo.name)">
...
const { fileOp } = fileViewProxy.value;
</script>
