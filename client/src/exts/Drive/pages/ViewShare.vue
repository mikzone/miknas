<template>
  <LayoutHeader>
    <template #toolbar>
      <template v-if="state.shareInfo">
        <div
          class="row no-wrap items-center justify-start q-gutter-sm mn-toolbar-title"
  ...
  <q-page-container>
    <q-page>
      ...
      ></MdcDriveAliveView>
      <div v-else-if="state.needPwd" class="absolute-full flex-center q-pa-md row q-col-gutter-sm">
        <div class="col-12 col-sm-6 col-md-4">
          <q-input v-model="state.inputPwd" filled label="请输入提取码" @keyup.enter="tryVerify" />
          <q-btn size="lg" class="q-mt-lg full-width" color="primary" @click="tryVerify"
            >提取文件</q-btn
          >
        </div>
      </div>
      <div v-else-if="state.errInfo" class="text-red absolute-full column flex-center">
        {{ state.errInfo.why }}
      </div>
